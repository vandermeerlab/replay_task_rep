%% Ensemble splitting
for c_i = 1:2
    left_preCP_correct = cellfun(@(x) x.left_preCP_correct{c_i}, out);
    left_postCP_correct = cellfun(@(x) x.left_postCP_correct{c_i}, out);
    right_preCP_correct = cellfun(@(x) x.right_preCP_correct{c_i}, out);
    right_postCP_correct = cellfun(@(x) x.right_postCP_correct{c_i}, out);

    left_chance_pre = cellfun(@(x) x.left_chance_pre{c_i}, out);
    left_chance_post = cellfun(@(x) x.left_chance_post{c_i}, out);
    right_chance_pre = cellfun(@(x) x.right_chance_pre{c_i}, out);
    right_chance_post = cellfun(@(x) x.right_chance_post{c_i}, out);
