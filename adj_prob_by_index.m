...
xpad = 0.5;
h = errorbar(x, adj_p_switch_m(x), adj_p_switch_sem(x), 'LineWidth', 1.5); hold on;
set(h, 'Color', 'k');
...
sh = shadedErrorBar(x, adj_p_switch_shuffles_m(x), [u_bound(x);l_bound(x)]);

% sh = errorbar(x, shuf_adj_p_switch_m, shuf_adj_p_switch_sem, 'LineWidth', 1.5); hold on;
% set(sh, 'Color', [0.7 0.7 0.7]);

hold on;
plot(x, adj_p_switch_m(x), '.k', 'MarkerSize', 20);
